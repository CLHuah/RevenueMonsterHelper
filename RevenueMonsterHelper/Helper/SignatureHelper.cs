...
public static class SignatureHelper
{
    ...
    public static string GenerateCompactJsonFromRaw(string json)
    {
        ...
        // Parse without altering any values, then write the canonical form
        var builder = new StringBuilder(json.Length);
        WriteCanonicalJson(builder, RawJson.Parse(json));
        ...
        string requestUrl, string signType, string timestamp)
    {
        ...
        var signatureInput = BuildSignatureInput(compactJson, method, nonceStr, requestUrl, NormalizeSignType(signType),
            timestamp);
    ...
    /// <summary>
    ///     Signs a request and returns the body to send together with the values for the signature headers.
    /// </summary>
    /// <param name="body">The request body, or null when the request has none.</param>
    /// <param name="method">The HTTP method (any casing).</param>
    /// <param name="requestUrl">The full request URL, including any query string, exactly as it is sent.</param>
    /// <param name="privateKey">The RSA private key in PEM format used for signing.</param>
    /// <returns>The signed request, with a new 32-character nonce and the current Unix time as the timestamp.</returns>
    /// <remarks>
    ///     Send <see cref="SignedRequest.Body" /> unchanged as the request body so that it always matches the
    ///     signature, and set the X-Nonce-Str, X-Timestamp and X-Signature headers from the result.
    /// </remarks>
    public static SignedRequest SignRequest(object body, string method, string requestUrl, string privateKey)
    {
        return SignRequest(body, method, requestUrl, privateKey, RandomString.GenerateRandomString(NonceLength),
            DateTimeOffset.UtcNow.ToUnixTimeSeconds().ToString(CultureInfo.InvariantCulture));
    }

    /// <summary>
    ///     Signs a request with the given nonce and timestamp and returns the body to send together with the values
    ///     for the signature headers.
    /// </summary>
    /// <param name="body">The request body, or null when the request has none.</param>
    /// <param name="method">The HTTP method (any casing).</param>
    /// <param name="requestUrl">The full request URL, including any query string, exactly as it is sent.</param>
    /// <param name="privateKey">The RSA private key in PEM format used for signing.</param>
    /// <param name="nonceStr">A random string to prevent replay attacks.</param>
    /// <param name="timestamp">The request timestamp (Unix time in seconds).</param>
    /// <returns>The signed request.</returns>
    public static SignedRequest SignRequest(object body, string method, string requestUrl, string privateKey,
        string nonceStr, string timestamp)
    {
        // The canonical JSON is both what gets signed and what gets sent
        var compactJson = body is null ? null : GenerateCompactJson(body);
        ...
        return new SignedRequest
        {
            ...
            NonceStr = nonceStr,
            Timestamp = timestamp,
            Signature = GenerateSignature(compactJson, method, nonceStr, privateKey, requestUrl, SupportedSignType,
                timestamp)
        };
    }
    ...
    /// <summary>
    ///     Verifies an incoming Revenue Monster request, such as a webhook, from its raw body and headers.
    /// </summary>
    /// <param name="rawBody">The request body exactly as received, or null/empty when there is none.</param>
    /// <param name="method">The HTTP method of the request (any casing).</param>
    /// <param name="requestUrl">The full URL the request was sent to, such as your notify URL.</param>
    /// <param name="nonceStr">The X-Nonce-Str header value.</param>
    /// <param name="timestamp">The X-Timestamp header value.</param>
    /// <param name="signatureHeader">The X-Signature header value, in the form "sha256 {signature}".</param>
    /// <param name="publicKey">Revenue Monster's RSA public key in PEM format.</param>
    /// <returns>
    ///     True if the signature is valid; false otherwise, including when the header is malformed or names an
    ///     unsupported sign type.
    /// </returns>
    public static bool VerifyWebhook(string rawBody, string method, string requestUrl, string nonceStr,
        string timestamp, string signatureHeader, string publicKey)
    {
        // The header is "{signType} {signature}"
        var parts = signatureHeader?.Trim().Split(' ', 2, StringSplitOptions.TrimEntries) ?? [];
        if (parts.Length != 2 || !string.Equals(parts[0], SupportedSignType, StringComparison.OrdinalIgnoreCase))
            return false;

        return VerifySignatureFromRawBody(rawBody, method, nonceStr, publicKey, requestUrl, parts[0], timestamp,
            parts[1]);
    }
    ...
    ///     "data={base64_encoded_json}&amp;method={method}&amp;nonceStr={nonceStr}&amp;requestUrl={requestUrl}&amp;
        ...
        string signType, string timestamp)
    {
        ...
        [
            ("data", data), ("method", method?.ToLowerInvariant()), ("nonceStr", nonceStr),
            ("requestUrl", requestUrl), ("signType", signType), ("timestamp", timestamp)
        ];

        // Join the non-empty parameters as key=value pairs
        return string.Join("&",
            parameters.Where(parameter => !string.IsNullOrEmpty(parameter.Value))
