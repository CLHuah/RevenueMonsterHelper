...
#pragma warning restore CS0618
    ...
    public void RefundRequest_MatchesDocumentedRequestBody()
    {
        ...
        var request = new RefundRequest
        {
            ...
            {
                type = RefundTypes.Full, currencyType = CurrencyTypes.MalaysianRinggit, amount = 100
            },
            reason = "test"
        };

        // Act & Assert
        Assert.AreEqual(SignatureHelper.GenerateCompactJsonFromRaw(documented),
            SignatureHelper.GenerateCompactJson(request));
    }
    ...
    public void WebPayment_OptionalPropertiesSet_AreSentWithoutNullFields()
    {
        ...
        // Assert
        Assert.Contains("\"customer\":{\"email\":\"a@example.com\"}", json);
        Assert.Contains("\"expiresInSeconds\":600", json);
        Assert.Contains("\"voucher\":{\"code\":\"V1\"}", json);
    }
