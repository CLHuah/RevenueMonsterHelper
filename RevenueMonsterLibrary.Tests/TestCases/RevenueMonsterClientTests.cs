...
public class RevenueMonsterClientTests
{
    ...
    {
        // Arrange
        var (client, _) = CreateClient(CreateOptions(),
            request => request.IsTokenRequest
    ...
    public async Task ApiRequest_NonJsonResponse_ThrowsWithStatusCodeAndBody()
    {
        ...
        var (client, _) = CreateClient(CreateOptions(),
            request => request.IsTokenRequest
                ...
                {
                    Content = new StringContent("<html>502</html>")
                });
    ...
    public async Task ApiRequest_Unauthorized_DropsCachedTokenForNextRequest()
    {
        ...
        var (client, handler) = CreateClient(CreateOptions(),
            request => request.IsTokenRequest ? Ok(tokenResponses.Dequeue()) : apiResponses.Dequeue());
    ...
    public async Task ProductionEnvironment_UsesProductionUrls()
    {
        ...
        var (client, handler) = CreateClient(options,
            request => request.IsTokenRequest ? Ok(FirstTokenResponse) : Ok("""{"item":{},"code":"SUCCESS"}"""));
    ...
    private static RevenueMonsterOptions CreateOptions()
    {
        return new RevenueMonsterOptions
        {
            ...
            ClientId = Guid.NewGuid().ToString("N"), ClientSecret = "secret", PrivateKey = TestKeys.PrivateKey
        };
    }
